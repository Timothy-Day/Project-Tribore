...
backward.1 = lm(SalePrice ~ MSSubClass + LotArea + OverallQual + OverallCond + 
  ...
  Fireplaces + GarageArea + WoodDeckSF + ScreenPorch + PoolArea + 
  YrsAge + MSZoning + LandSlope + Neighborhood + RoofMatl + 
  BsmtExposure + Heating + HeatingQC + CentralAir + KitchenQual + 
  Functional + SaleCondition + Condition, data = housing_new2[!row.names(housing_new2) %in% c("524", "826"),])


summary(backward.1)
plot(backward.1)
influencePlot(backward.1)

vif(backward.1)


############### Best Model based on the full training data set ###################################################
...
forward.1 = lm(SalePrice ~ OverallQual + GrLivArea + Neighborhood + TotalBsmtSF + 
                 ...
                 LotArea + MSZoning + Condition + BsmtExposure + KitchenQual + 
                 Fireplaces + Functional + SaleCondition + CentralAir + BldgType + 
                 ScreenPorch + Heating + LandSlope + WoodDeckSF + GarageArea + 
                 PoolArea + HeatingQC + BsmtHalfBath + BedroomAbvGr, data = housing_new2[!row.names(housing_new2) %in% c("524", "826"),])
...
forward.1 = lm(SalePrice ~ OverallQual + GrLivArea + Neighborhood + TotalBsmtSF + 
                 ...
                 Fireplaces + Functional + SaleCondition + CentralAir + BldgType + 
                 ScreenPorch + Heating + LandSlope + WoodDeckSF + GarageArea + 
                 PoolArea + HeatingQC + BsmtHalfBath + BedroomAbvGr, data = housing_new2)
...
as.data.frame(table(unlist(housing_new2$Condition)))
...
test_new2$Utilities = test_new2$Utilities %>% 
  fct_collapse(AllPub = c("None", "AllPub"))

test_new2$SaleType = test_new2$SaleType %>% 
  fct_collapse(Oth = c("None", "Oth"))
...
write.xlsx2(y.predict3, file = "submission_v2.xlsx", sheetName = "Sheet1",
            col.names = TRUE, row.names = TRUE, append = FALSE)
